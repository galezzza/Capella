...
vec3 get_color(vec3 newLightDir, float exp_param){
	...
	vertexTexCoord.y = 200.f - vertexTexCoord.y;
	...
	vec3 N = texture2D(normalSampler, vVertexTexCoord).rgb*2-1;

	N=normalize(N+vec3(0,0,1)*10);
	//N=vec3(0,0,1);

	vec3 normalizeLightDir = normalize(newLightDir);
	vec3 normalizeVertexNormal = normalize(N);
	...
	vec3 VVertex = normalize(viewDirTS);
...
void main()
{
	vec3 color = texture2D(colorTexture, vVertexTexCoord).rgb;
	...
	vec3 combined_illumination = get_color(lightDirTS, exp_param);
	
	vec3 spotLightDir = ioVertexPosition - spotPos;
	if (dot(normalize(spotLightDir), normalize(spotDir)) > cos(angleAf)){
		//combined_illumination = combined_illumination + get_color(spotLightDir, exp_param/5);
	}
