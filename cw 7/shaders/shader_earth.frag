...
vec3 get_color(vec3 newLightDir, float exp_param){
	...
	vec3 earthColor = texture2D(colorTexture, vertexTexCoord).rgb;
	vec3 cloudsColor = texture2D(clouds, vertexTexCoord).rgb;
	vec3 color = mix(earthColor,vec3(1,1,1), cloudsColor.r);
