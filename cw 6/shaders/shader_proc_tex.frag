...
vec3 get_color(vec3 newLightDir, float exp_param, vec3 color){
	float diffuse;
	float intensity;
...
void main()
{
	...
	vec3 newLightDir = ioVertexPosition - lightPos;
	vec3 combined_illumination;
	
	vec3 newColor;

	if(sin(ioLocalVertexPosition.y)>0){
		newColor = vec3(1,0,0);
	}
	else{
		newColor = vec3(0,0,1);
	}
	combined_illumination = get_color(newLightDir, exp_param, newColor);
	
	vec3 spotLightDir = ioVertexPosition - spotPos;
	if (dot(normalize(spotLightDir), normalize(spotDir)) > cos(angleAf)){
		combined_illumination = combined_illumination + get_color(spotLightDir, exp_param/5, newColor);
	}
